...
(function (win, $) {
    ...
    $('.go_hp').on('click', function () {
        $("html,body").animate({scrollTop: "1400px"}, 200);
    });
    // 回顶部
    ...
    $(window).scroll(function () {
        ...
        if(_top>650&&_top<2200){
            $('.go_hp').css('display', 'none');
        } else {
            $(".go_hp").css('display', 'inline-block');
        }
    });
    /*预约表单*/
    ...
    ReserveForm.prototype = {
        ...
        submit : function(){
            ...
            var userAppellation = $form.find('[name="name"]').val();
            var phoneNum = $form.find('[name="phoneNumber"]').val();
            var bookingNote = 'iPhone/' + model + ':' + fault+'故障,手机换屏';   // 故障说明
    ...
    $('#in_screen').on('click', function () {
        if ($('#in_screen').hasClass('select_tab') == false) {
            $('#in_screen').removeClass('unselect_tab');
            $('#in_screen').addClass('select_tab');
        }
        if ($('#out_screen').hasClass('unselect_tab') == false) {
            $('#out_screen').removeClass('select_tab');
            $('#out_screen').addClass('unselect_tab');
        }
        $('.grid2').css('display', 'inline-block');
        $('.grid').css('display', 'none');
    });
    $('#out_screen').on('click', function () {
        if ($('#in_screen').hasClass('unselect_tab') == false) {
            $('#in_screen').removeClass('select_tab');
            $('#in_screen').addClass('unselect_tab');
        }
        if ($('#out_screen').hasClass('select_tab') == false) {
            $('#out_screen').removeClass('unselect_tab');
            $('#out_screen').addClass('select_tab');
        }
