...
layui.use(['jquery', 'simplePager', 'laydate', 'form', 'element', 'layer', 'cookie', 'global', 'upmobui', 'region'], function () {
    ...
    Page.prototype = {
                    ...
                    if (undefined != data.data && null != data.data && data.code == '200') {
                        ...
                        if (len > 0) {
                            for (var i = 0; i < len; i++) {
                                html += '<tr>';
                                html += '<td>' + (i - 0 + 1) + '</td>';
                                html += '<td><a href="javascript:void(0);" data-id="' + dataList[i].id + '" >' + dataList[i].name + '</a></td>';
                                ...
                                html += '<td>'
                                    ...
                                    + '<a data-ct="删除这个客服账号" href="javascript:void(0);" data-id="' + dataList[i].id + '" class="btn_delete">删除</a>'
                                    + '</td>';
                                html += '</tr>';
                            }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (undefined != data && null != data && data.code == '200') {
                                ...
                                layer.msg(flagmsg + '失败！客服账号已存在', { time: 1500 },function () {
        ...
        checkForm: function () {
            ...
            if (!name) {
                layer.msg('客服账号名称不能为空！', { time: 1200 });
                return false;
            }
            var mobile = $('.layui-layer-content [name=mobile]').val();
            if (!mobile) {
                layer.msg('客服账号电话不能为空！', { time: 1200 });
                return false;
            }
